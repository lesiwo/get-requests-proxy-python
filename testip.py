...
def main(inPut="d:/python/爬虫/ips.json",outPut="d:/python/爬虫/goodips.json"):
    global testnum,testing,testOverNum,all
    ...
    for data in js:
        testing.append(threading.Thread(target=test, args=(data,)))

    for i in testing:
        i.setDaemon(True)

    while True:
        if testnum ==len(testing)-1:
            break
        if threading.active_count() < 257:
            testing[testnum].start()
            testnum += 1
